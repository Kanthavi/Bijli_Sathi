...
function LoginScreen({ language, setLanguage, onLogin }) {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "#f8faff", alignItems: "center", justifyContent: "center", fontFamily: "system-ui, sans-serif" }}>
      <div style={{ width: 420, padding: 32, background: "#fff", borderRadius: 24, boxShadow: "0 10px 40px rgba(0,102,255,0.08)" }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          <div style={{ fontSize: 56, marginBottom: 12 }}>⚡</div>
          <div style={{ fontSize: 28, fontWeight: 800, color: "#0A0F2C" }}>Bijli Sathi</div>
          <div style={{ fontSize: 13, color: "#8892A4", fontWeight: 600, marginTop: 4 }}>Save Power. Save Money.</div>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#8892A4", textTransform: "uppercase", marginBottom: 8, letterSpacing: 0.5 }}>Email Address</div>
            <input type="email" placeholder="e.g. aditi.sharma@gmail.com" value={email} onChange={e => setEmail(e.target.value)}
              style={{ width: "100%", padding: "14px 16px", borderRadius: 14, border: `2px solid #E8ECF4`, background: "#F5F7FF", outline: "none", fontSize: 15, fontWeight: 600, color: "#0A0F2C" }} />
          </div>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#8892A4", textTransform: "uppercase", marginBottom: 8, letterSpacing: 0.5 }}>OTP Verification</div>
            <div style={{ display: "flex", gap: 8 }}>
              <input type="tel" placeholder="Enter 6-digit OTP" style={{ flex: 1, padding: "14px 16px", borderRadius: 14, border: `2px solid #E8ECF4`, background: "#F5F7FF", outline: "none", fontSize: 15, fontWeight: 600, color: "#0A0F2C", width: "100%" }} defaultValue="" />
              <button style={{ padding: "14px 20px", background: "rgba(0,102,255,0.08)", color: "#0066FF", border: "none", borderRadius: 14, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Send OTP</button>
            </div>
          </div>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#8892A4", textTransform: "uppercase", marginBottom: 8, letterSpacing: 0.5 }}>Select Your DISCOM</div>
            <select style={{ width: "100%", padding: "14px 16px", borderRadius: 14, border: `2px solid #E8ECF4`, background: "#F5F7FF", outline: "none", fontSize: 14, fontWeight: 600, color: "#0A0F2C", cursor: "pointer" }}>
              <option>BSES Rajdhani (Delhi)</option>
              <option>TATA Power (Mumbai)</option>
              <option>BESCOM (Bengaluru)</option>
              <option>TNEB (Tamil Nadu)</option>
              <option>MSEDCL (Maharashtra)</option>
            </select>
          </div>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#8892A4", textTransform: "uppercase", marginBottom: 8, letterSpacing: 0.5 }}>Preferred Language</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
              {["English", "हिंदी", "বাংলা", "தமிழ்", "తెలుగు", "मराठी"].map(l => (
                <div key={l} onClick={() => setLanguage(l)} style={{ padding: "12px 10px", borderRadius: 12, border: `2px solid ${language === l ? "#0066FF" : "#E8ECF4"}`, textAlign: "center", fontSize: 13, fontWeight: 600, cursor: "pointer", background: language === l ? "rgba(0,102,255,0.08)" : "#fff", color: language === l ? "#0066FF" : "#0A0F2C", transition: "all 0.2s" }}>{l}</div>
              ))}
            </div>
          </div>
          <button onClick={handleLogin} style={{ width: "100%", padding: 18, borderRadius: 16, background: "linear-gradient(135deg, #0055EE, #0077FF)", color: "#fff", border: "none", fontSize: 16, fontWeight: 800, cursor: "pointer", boxShadow: "0 8px 24px rgba(0,102,255,0.3)", marginTop: 12, transition: "transform 0.2s" }}>Continue to Dashboard →</button>
        </div>
      </div>
    </div>
  );
...
export default function BijliSathi() {
  ...
  const chip = (c) => ({
    display: "inline-flex", padding: "3px 9px", borderRadius: 20, fontSize: 10, fontWeight: 700,
  ...
  const Dashboard = () => (
    <div>
      <div style={{ fontSize: 20, fontWeight: 800, marginBottom: 4 }}>Good Evening, {userName} 👋</div>
  ...
  if (!isLoggedIn) {
    return <LoginScreen language={language} setLanguage={setLanguage} onLogin={(name, email) => {
      setUserName(name);
      setUserEmail(email);
      setIsLoggedIn(true);
    }} />;
    ...
    <div style={{ display: "flex", height: "100vh", background: C.bg, color: C.text, fontFamily: "system-ui,sans-serif", overflow: "hidden" }}>
      ...
      <aside style={{ width: 210, background: C.bg2, borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column", flexShrink: 0 }}>
        ...
        <nav style={{ padding: "6px 0", flex: 1, overflowY: "auto" }}>
          {TABS.map(t => (
            <div key={t.id} style={{
              display: "flex", alignItems: "center", gap: 9, padding: "9px 16px", cursor: "pointer",
        ...
        <div style={{ margin: "0 10px 12px", padding: "10px 12px", background: C.bg3, borderRadius: 12, border: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <button 
            onClick={() => { setIsLoggedIn(false); setUserName("User"); setUserEmail(""); }} 
            title="Logout"
            style={{ background: "transparent", border: "none", color: C.text3, cursor: "pointer", padding: 4, display: "flex" }}
          >
